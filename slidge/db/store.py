...
class ContactStore(EngineMixin):
    ...
    def add(self, user_pk: int, legacy_id: str, contact_jid: JID) -> None:
        with self.session() as session:
            contact = Contact(
    ...
    def get_presence(self, contact_jid: JID, user_pk: int) -> Optional[CachedPresence]:
        with self.session() as session:
    ...
    def set_presence(
        self, contact_jid: JID, user_pk: int, presence: CachedPresence
    ) -> None:
        with self.session() as session:
            session.execute(
                update(Contact)
    ...
    def reset_presence(self, contact_jid: JID, user_pk: int):
        with self.session() as session:
            session.execute(
                update(Contact)
                .where(Contact.jid == contact_jid.bare)
...
class MAMStore(EngineMixin):
    ...
    def add_message(self, db_id: str, message: HistoryMessage, user_pk: int) -> None:
        with self.session() as session:
            room_id = session.execute(
                select(Room.id)
                .where(Room.jid == db_id)
            ...
            try:
                ...
                session.execute(
                    ...
                    .where(ArchivedMessage.user_account_id == user_pk)
                    .where(ArchivedMessage.stanza_id == message.id)
                    .where(ArchivedMessage.room_id == room_id)
                    .values(
                        stanza_id=message.id,
                        timestamp=message.when,
                        stanza=str(message.stanza),
    ...
    def __room_id(self, user_pk: int, jid: str) -> Optional[int]:
        with self.session() as session:
            return session.execute(
                select(Room.id)
                .where(Room.jid == jid)
    ...
    def get_messages(
        ...
        with self.session() as session:
            q = (
                ...
                .where(ArchivedMessage.room_id == self.__room_id(user_pk, db_id))
                .where(ArchivedMessage.user_account_id == user_pk)
            )
            ...
            if before_id is not None:
                stamp = session.execute(
                    select(ArchivedMessage.timestamp)
                    .where(ArchivedMessage.stanza_id == before_id)
                    .where(ArchivedMessage.user_account_id == user_pk)
                ).scalar()
                if stamp is None:
                    raise XMPPError(
            ...
            if after_id is not None:
                stamp = session.execute(
                    select(ArchivedMessage.timestamp)
                    .where(ArchivedMessage.stanza_id == after_id)
                    .where(ArchivedMessage.user_account_id == user_pk)
                ).scalar()
                if stamp is None:
                    raise XMPPError(
    ...
    def get_first_and_last(self, db_id: str, user_pk: int) -> list[MamMetadata]:
        r = []
        with self.session() as session:
...
class MultiStore(EngineMixin):
    ...
    def set_xmpp_ids(
        self, user_pk: int, legacy_msg_id: str, xmpp_ids: list[str]
    ) -> None:
        with self.session() as session:
            row = LegacyIdsMulti(
                user_account_id=user_pk,
            ...
            try:
                ...
                session.commit()
                self.set_xmpp_ids(user_pk, legacy_msg_id, xmpp_ids)
