...
class GatewayUser(Base):
    ...
    archived_messages: Mapped[list["ArchivedMessage"]] = relationship(
...
class ArchivedMessage(Base):
    ...
    __tablename__ = "mam"
    __table_args__ = (UniqueConstraint("user_account_id", "room_id", "stanza_id"),)

    id: Mapped[int] = mapped_column(primary_key=True)
    user_account_id: Mapped[int] = mapped_column(ForeignKey("user_account.id"))
    user: Mapped[GatewayUser] = relationship()
