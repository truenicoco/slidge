...
class Session(BaseSession[str, Recipient]):
    ...
    async def handle_chat_state(self, state: whatsapp.ChatState):
        contact = await self.get_contact_or_participant(
            state.JID, state.GroupJID, False
        )
    ...
    async def handle_receipt(self, receipt: whatsapp.Receipt):
        ...
        """
        contact = await self.get_contact_or_participant(
            receipt.JID, receipt.GroupJID, receipt.IsCarbon
        )
    ...
    async def handle_message(self, message: whatsapp.Message):
        ...
        """
        contact = await self.get_contact_or_participant(
            message.JID, message.GroupJID, message.IsCarbon
        )
        ...
        message_reply_to = message.OriginJID if message.OriginJID else None
        message_timestamp = (
    ...
    async def get_contact_or_participant(
        self, legacy_contact_id: str, legacy_group_jid: str, is_carbon: bool
    ):
        """
        ...
        if legacy_group_jid:
            ...
            if is_carbon:
                ...
                contact = await self.contacts.by_legacy_id(legacy_contact_id)
                return await muc.get_participant_by_contact(contact)
        else:
